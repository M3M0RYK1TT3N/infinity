...
elseif (isset($_POST['post'])) {
	...
	if ($post['has_file']) {
		...
		foreach ($_FILES as $key => $file) {
			if ($file['size'] && $file['tmp_name']) {
				$file['filename'] = urldecode(function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() ? stripslashes($file['name']) : $file['name']);
	...
	if ($post['has_file']) {
		...
		if ($file['is_an_image'] && $config['ie_mime_type_detection'] !== false) {
			...
			if ($config['redraw_image'] || (empty($file['exif_stripped']) && $config['strip_exif'] && ($file['extension'] == 'jpg' || $file['extension'] == 'jpeg'))) {
